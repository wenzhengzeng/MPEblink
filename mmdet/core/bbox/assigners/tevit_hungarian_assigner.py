...
class TeViTHungarianAssigner(BaseAssigner):
    ...
    def assign(self,
        ...
        for i in range(clip_length): # for each frame
            ...
            # regression L1 cost
            normalize_gt_bboxes = gt_bboxes[i] / factor     # gt is in the form of [x_1, y_1, x_2, y_2], divided by the image resolution for normalization
        ...
        for gt_id in gt_ids:
            ...
            totals_to_per_ims = gt_id.new_zeros(num_gts + 1, dtype=torch.int64) # num_gt+1 the first dimension is for none-object, others are the true gts
        ...
        cost = sum(costs_) / sum(ims_to_total_weights) # summarize to form the video-level cost between each spatio-temporal query and gt
        ...
        matched_row_inds, matched_col_inds = linear_sum_assignment(cost) # row_inds: the index of the matched proposal, col_inds: the matched gt index corresponding to those proposals
        ...
        for _ in range(clip_length):    # for each frame
            ...
            matched_col_inds_per_im = matched_col_inds_per_im[
                matched_col_inds_per_im > 0]
            assigned_gt_inds[matched_row_inds_per_im] = matched_col_inds_per_im # record matched gt index to each proposal, 0 means none-object
