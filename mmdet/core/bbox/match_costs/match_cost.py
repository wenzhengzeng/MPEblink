...
class BBoxL1Cost:
    ...
    def __call__(self, bbox_pred, gt_bboxes):
        ...
        bbox_cost = torch.cdist(bbox_pred, gt_bboxes, p=1) # p-norm
        return bbox_cost * self.weight

@MATCH_COST.register_module()
class InferenceBBoxL1Cost:
    """BBoxL1Cost.
...
class FocalLossCost:
    ...
    def _focal_loss_cost(self, cls_pred, gt_labels):
        ...
        neg_cost = -(1 - cls_pred + self.eps).log() * (
            1 - self.alpha) * cls_pred.pow(self.gamma)  # -0.75* p^r * log(1-p) # The closer p is to 0, the smaller the loss
        pos_cost = -(cls_pred + self.eps).log() * self.alpha * (
            1 - cls_pred).pow(self.gamma)  # -0.25* (1-p)^2 * log(p) The larger the p, the smaller the loss

        cls_cost = pos_cost[:, gt_labels] - neg_cost[:, gt_labels]
        # The larger the p of the class, the smaller the loss cost of the class, regardless of how many classes there are, because they are counted within their respective classes
        # argmin will be used in the subsequent Hungarian match, which means that the smaller the cost, the more it will be matched.
