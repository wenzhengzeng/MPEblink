...
class InstBlink(QueryInst):
    ...
    def extract_feat(self, B, T, img):
        ...
        if hasattr(self.backbone, 'msg_tokens'):
            x = self.backbone(B, T, img) # features from 4 layers (ResNet)
        else:
            x = self.backbone(img)
        if self.with_neck:
            x = self.neck(x) # return features in 4 different level from FPN
        return x
    ...
    def forward_train(self,
        ...
        B, T = img.size()[:2]  # [b,clip_len,3,w,h]
        img = img.resize(B * T, *img.size()[2:]) # [b, clip_len, 3, w, h] --> [b*clip_len, 3, w, h]
        img_metas = [_ for clip in img_metas for _ in clip]
        gt_bboxes = [_ for clip in gt_bboxes for _ in clip] # Some frames have multiple instances, then there will be multiple gt boxes
        gt_labels = [_ for clip in gt_labels for _ in clip]
        gt_blinks = [_ for clip in gt_blinks for _ in clip]
        gt_ids = [_ for clip in gt_ids for _ in clip]

        x = self.extract_feat(B, T, img) # x: features after neck (e.g.,FPN)
