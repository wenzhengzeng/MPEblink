...
class MsgShifTBase(BaseModule):
    ...
    def forward(self, B, T, x):
        outs = []
        msg_tokens = self.msg_tokens.repeat(B * T, 1, 1) # [1,num_msg_tokens(32),in_channels(3)] --> [b*clip_len,num_msg_tokens(32),in_channels(3)]这个三只是初始化，后面会逐渐增大

        for i, layer in enumerate(self.layers):
            x, hw_shape, msg_tokens = layer[0](x, msg_tokens) # 这个layer[0]对应的是PatchEmbed 应该就是一个卷积
            # x:[b*t,降采样后的h*w,64] hw_shape：原输入长宽都降采样4分之一，msg_tokens:[b*t,num_msg_tokens(32),通道变多]
            for _, block in enumerate(layer[1]): # 这里layer[1]对应连续两层PVTEncoderLayer
                x, msg_tokens = block(x, B, T, hw_shape, msg_tokens, msg_shift=self.msg_shift[i][_]) # block是PVTEncoderLayer,这里x,msg_tokens的维度不变
            x, msg_tokens = layer[2](x), layer[2](msg_tokens) # x和msg_tokens分别做一个layer norm
            x = nlc_to_nchw(x, hw_shape) # 变成w*h的形态 [b*t, c, w, h]
            if i in self.out_indices:  # 收集4个大layer的特征，用于FPN吧
                outs.append(x)
